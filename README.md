...
- `立即完整討論`：備用手動入口，執行與收盤相同的完整流程：日結、當沖 Agent、短線 Agent、multi-agent 檢討與策略版本套用。
...
多 Agent 檢討會建立 `review_evidence`、`review_runs`、`agent_reviews`、`strategy_proposals`，並把通過 validator / replay / risk gate 的結果寫成 `validated` strategy version。策略若處於 `follow_latest`，新版會直接成為 active；若你已手動鎖定舊版，系統會保留鎖定版本。

收盤日結會自動執行完整討論流程；`立即完整討論` 只是備用補跑入口。
...
- `立即完整討論`：使用最近資料重跑完整盤後流程；必要時建立並套用 `scout/daytrade/swing` 新版，手動鎖定舊版時不自動切換。
